...
export function runScene() {
    ...
    // --- GUI ---
    const gui = new GUI();
    gui.add(cameraConfig, 'lookAhead', 1, 20);
    gui.add(cameraConfig, 'cameraDistance', 5, 20);
    gui.add(cameraConfig, 'cameraHeight', 1, 10);
    gui.add(cameraConfig, 'positionSmooth', 0.01, 0.2);
    gui.add(cameraConfig, 'lookAtSmooth', 0.01, 0.2);
    ...
    function updateChaseCamera() {
        ...
        // 3. Smoothly interpolate the camera's actual position towards the target
        camera.position.lerp(cameraTarget, cameraConfig.positionSmooth);

        // 4. Smoothly interpolate the actual look-at point and apply it
        smoothedLookAt.lerp(lookAtTarget, cameraConfig.lookAtSmooth);
