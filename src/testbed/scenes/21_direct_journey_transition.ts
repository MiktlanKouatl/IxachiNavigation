...
import { RibbonLine, RibbonConfig, UseMode, FadeStyle } from '../../core/RibbonLine';
...
export function runScene() {
    console.log('🚀 Ixachi Components Testbed - Scene 21: Direct Journey Transition');
    ...
    const HOST_RIBBON_Y_OFFSET = -2.0;
    let hostRibbonStarted = false;
    ...
    for (let i = 0; i < 4; i++) {
        ...
        const ribbon: AnimatingRibbonLine = new ProceduralRibbonLine(pathData, {
            ...
            fadeStyle: FadeStyle.FadeInOut, // Use FadeIn
            fadeTransitionSize: 1, // Start with full transition
        });
    ...
    function startRibbonAnimation() {
        ribbons.forEach((ribbon, i) => {
            // Reset transition size to 1 before starting the animation
            ribbon.material.uniforms.uFadeTransitionSize.value = 20.0;
    ...
    const hostRibbonConfig: RibbonConfig = {
        ...
        fadeTransitionSize: 0.9,
    };
    const hostRibbon = new RibbonLine(hostRibbonConfig);
    scene.add(hostRibbon.mesh);

    const hostRibbonFolder = gui.addFolder('Host Ribbon');
    hostRibbonFolder.add(hostRibbon.material.uniforms.uFadeTransitionSize, 'value', 0.0, 1.0).name('Fade Transition Size');
    ...
    function animate() {
        ...
        if (!hostRibbonStarted && cameraFollower.position.z <= 0) {
            hostRibbonStarted = true;
        }

        if (hostRibbonStarted) {
            const ribbonPoint = cameraFollower.position.clone();
            ribbonPoint.z -= 20; // Apply Z offset
            ribbonPoint.y += HOST_RIBBON_Y_OFFSET;  // Apply Y offset
            hostRibbon.addPoint(ribbonPoint);
        }
    ...
    if (startButton) {
        ...
        startButton.addEventListener('click', () => {
            ...
            hostRibbon.reset();
            hostRibbonStarted = false;
            hostRibbon.addPoint(new THREE.Vector3(0, HOST_RIBBON_Y_OFFSET, 0)); // Add initial point at (0,y,0)
            clock = new THREE.Clock();
            startRibbonAnimation(); // Restart ribbon animation
            animate();
        });
    }
